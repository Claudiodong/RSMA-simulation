...
M_all = 10:10:50;                     % THe number of elements in RIS
SINR = 10.^(0./10);                   % SINR in linear
sigma = 10.^((-70-30)/10);            % The noise power
Max = 500;                            % The number fo channel realisation
...
% Optimal
power_SOCP = zeros(Max,length(M_all));
power_SDP  = zeros(Max,length(M_all));
power_KKT  = zeros(Max,length(M_all));

% Random
power_SOCP_random = zeros(Max,length(M_all));
power_SDP_random  = zeros(Max,length(M_all));
power_KKT_random  = zeros(Max,length(M_all));

% MU MISO
SOCP_MU_MISO = zeros(Max,length(M_all));
SDP_MU_MISO = zeros(Max,length(M_all));
KKT_MU_MISO = zeros(Max,length(M_all));
...
while(i_max<Max)
    ...
    % start the SINR loop
    for i_m = 1:length(M_all)
     % SINR value
     M = M_all(i_m); % set the M
     ...
     for i = 1:N_UE
        ...
        [power_SOCP(i_max,i_m)]=SOCP_optimisation_RIS(h_k_optimal,SINR,sigma);     % SOCP
        [power_SDP(i_max,i_m)]=SDP_optimisation_RIS(h_k_optimal,SINR,sigma);       % SDP
        [KKT_optimal]=KKT_optimisation_RIS(h_k_optimal,SINR,sigma,tolerance);        % KKT
        power_KKT(i_max,i_m) = sum(KKT_optimal);

        %% Random phase shift
        [power_SOCP_random(i_max,i_m)]=SOCP_optimisation_RIS(h_k_random,SINR,sigma);    
        [power_SDP_random(i_max,i_m)]=SDP_optimisation_RIS(h_k_random,SINR,sigma);       % SDP
        [KKT_randoml]=KKT_optimisation_RIS(h_k_random,SINR,sigma,tolerance);        % KKT
        power_KKT_random(i_max,i_m) = sum(KKT_randoml);

        %% MU MISO (checked)
        [SOCP_MU_MISO(i_max,i_m)]=SOCP_optimisation_MU_MISO(h_d_k,sigma,SINR);% SOCP
        [SDP_MU_MISO(i_max,i_m)]=SDP_optimisation_RIS(h_d_k,SINR,sigma);      % SDP    
        [KKT_MU]=KKT_optimisation_RIS(h_d_k,SINR,sigma,tolerance);              % KKT
        KKT_MU_MISO(i_max,i_m) = sum(KKT_MU);    
    end
end
...
%% plot the figures
figure()
plot(M_all,Avg_p_SOCP_optimal,"r-square",'LineWidth',1.5,'MarkerSize',13)
hold on;
plot(M_all,Avg_p_SDP_optimal,"g-O",'LineWidth',1.5,'MarkerSize',8)
hold on;
plot(M_all,Avg_p_KKT_optimal,"b-x",'LineWidth',1.5,'MarkerSize',8)
hold on;
plot(M_all,Avg_p_SOCP_random,"--square",'color',orange,'LineWidth',1.5,'MarkerSize',13)
hold on;
plot(M_all,Avg_p_SDP_random,"m--O",'LineWidth',1.5,'MarkerSize',8)
hold on;
plot(M_all,Avg_p_KKT_random,"k--x",'LineWidth',1.5,'MarkerSize',8)
hold on;
plot(M_all,Avg_p_SOCP_MU_MISO,"-.square",'LineWidth',1.5,'MarkerSize',13)
hold on;
plot(M_all,Avg_p_SOCP_MU_MISO,"-.O",'LineWidth',1.5,'MarkerSize',8)
hold on;
plot(M_all,Avg_p_SOCP_MU_MISO,"-.x",'LineWidth',1.5,'MarkerSize',8)
grid on;xlabel("Number of Elements in RIS");ylabel("Transmit Power [dBm]");
...
plot(M_all,Avg_p_SOCP_MU_MISO,"-.square",'LineWidth',1.5,'MarkerSize',13)
grid on;xlabel("Number of Elements in RIS");ylabel("Transmit Power [dBm]");
legend("Optimal Phase",'Random Phase','MU-MISO (NO-RIS)')

...
t_corr1 = [0.1,0.5,0.98];               % time correlation for channels (time correlation)
time_k = 1;                             % time instant (spatial correlation)
drop = 1e4;                             % Number of drop
Avg_C = zeros(drop,Num_User);
Avg_C_all = zeros(length(t_corr1),drop*Num_User);
...
for i_c = 1:length(t_corr1)
    for i_drop = 1:drop                 % Number of drop loop, where the UEs are at fixed location
        ...
        % Path loss in dB
        PL_dB_other_BS_uEs = 128.1+37.6*log10(interfer_BS_to_UEs./1000)+shadowing_dB*randn(Nr,Num_User,Num_BS);
        ...
        for i_time = 1:time_k % the number of time instant that the channel will vary when the UEs are fixed
             ...
             % up => MISO system, not inter-stream interference between the
             % stream
             Ne = min(min(Nt,Num_User),Q);
             Es_per_channel = (Tx_power/Ne); % transmit power per UE per channel
   ...
   Avg_C_all(i_c,:)=sort(reshape(Avg_C,1,[]));
end
%%
cdf = (1:length(Avg_C_all))./length(Avg_C_all)*100;

figure()
plot(Avg_C_all,cdf,"LineWidth",1.5);
grid on;
legend("\epsilon=0","\epsilon=0.5","\epsilon=0.98")

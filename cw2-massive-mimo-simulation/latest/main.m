...
Q = 2;                                  % the number of scheduled user at this time instant
...
%          end
         ...
         w_center1 = S_sqrt.*w_center;
         w_interfere1 = S_sqrt.*w_interfere;
...
%              Sum_C(i_t,Scheduled_User(i_e)) = C(i_e);
%          end

         C = Capacity1(center_H,H_interfere,w_center1,w_interfere1,N0);
...
%save("NT4Q2.mat","Avg_sort");

function C = Capacity1(center_H,H_interfere,w_center,w_interfere,N0)
         ...
         for i_bs = 1:Num_BS
             ...
             I_Interfere_all(:,i_bs) = sum(abs(L).^2,2);
         end

         % Compute the SINR for the User
         UE_all_power = abs(center_H*w_center).^2;
         UE_power = diag(UE_all_power);
         SINR = zeros(1,Q);
         C = zeros(1,Q);

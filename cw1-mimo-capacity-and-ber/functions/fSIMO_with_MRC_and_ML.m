...
%>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
% SIMO system with Maximum Ratio Transmission at transmitter side
...
function [error] = fSIMO_with_MRC_and_ML(SNR,Tx_QPSK_symbol,QPSK_constellation,Nt,Nr)
        ...
        % overall received signal at receivers
        y = p*H*Tx_QPSK_symbol+noise; % same as [y1;y2]
        % g matrix for MRC
        g = H'*norm(H);
        % Applying MRC
        Z = g*y;
        ...
        for i = 1:Num_symb
            ...
            [~,index] = min(abs(Z(i)-QPSK_constellation));
